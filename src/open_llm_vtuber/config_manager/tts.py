...
class GPTSoVITSConfig(I18nMixin):
    ...
    auto_start_server: bool = Field(False, alias="auto_start_server")
    gpt_sovits_root: str = Field("", alias="gpt_sovits_root")
    startup_timeout_sec: int = Field(60, alias="startup_timeout_sec")
    auto_start_server: bool = Field(False, alias="auto_start_server")
    gpt_sovits_root: str = Field("", alias="gpt_sovits_root")
    startup_timeout_sec: int = Field(60, alias="startup_timeout_sec")
    auto_start_server: bool = Field(False, alias="auto_start_server")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "auto_start_server": Description(
            en="Automatically start local GPT-SoVITS server if API is unreachable",
            zh="当 API 不可达时自动启动本地 GPT-SoVITS 服务",
        ),
        "gpt_sovits_root": Description(
            en="Path to GPT-SoVITS project root (auto-detect when empty)",
            zh="GPT-SoVITS 项目根目录路径（留空则自动检测）",
        ),
        "startup_timeout_sec": Description(
            en="Seconds to wait for GPT-SoVITS API to become ready",
            zh="等待 GPT-SoVITS API 就绪的秒数",
        ),
    }
...
class F5TTSConfig(I18nMixin):
    ...
    vocab_path: str = Field(..., alias="vocab_path")
    ref_audio_path: str = Field(..., alias="ref_audio_path")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "model_type": Description(
            en="Type of the TTS model (e.g., 'fastspeech2', 'tacotron2')",
            zh="TTS 模型的类型（例如 'fastspeech2'，'tacotron2'）"
        ),
        "model_path": Description(
            en="Path to the TTS model file",
            zh="TTS 模型文件的路径"
        ),
        "vocab_path": Description(
            en="Path to the vocabulary file used by the model",
            zh="模型使用的词汇表文件的路径"
        ),
        "ref_audio_path": Description(
            en="Path to reference audio for voice cloning or style transfer",
            zh="用于语音克隆或风格迁移的参考音频路径"
        ),
        "ref_text": Description(
            en="Reference text corresponding to the reference audio",
            zh="与参考音频对应的参考文本"
        ),
        "device": Description(
            en="Device to run the TTS model on (e.g., 'cpu', 'cuda')",
            zh="运行 TTS 模型的设备（例如 'cpu'，'cuda'）"
        ),
...
class TTSConfig(I18nMixin):
    ...
    f5_tts: Optional[F5TTSConfig] = Field(None, alias="f5_tts")  # Add f5_tts field
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "f5_tts": Description(
            en="Configuration for F5 TTS", zh="F5 TTS 配置"
        ),  # Add f5_tts description
    }
