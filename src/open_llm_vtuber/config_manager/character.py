...
class CharacterConfig(I18nMixin):
    ...
    live2d_model_name: str | None = Field(default=None, alias="live2d_model_name")
    live3d_model_name: str | None = Field(default=None, alias="live3d_model_name")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "live3d_model_name": Description(
            en="Name of the Live3D model to use", zh="使用的Live3D模型名称"
        ),
    ...
    def ensure_live_model_name(cls, values):
        if values is None:
            return values

        if values.get("live2d_model_name") is None and values.get("live3d_model_name") is not None:
            values["live2d_model_name"] = values["live3d_model_name"]

        if values.get("live3d_model_name") is None and values.get("live2d_model_name") is not None:
            values["live3d_model_name"] = values["live2d_model_name"]
