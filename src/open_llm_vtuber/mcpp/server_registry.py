...
class ServerRegistry:
    ...
    def __init__(self, config_path: str | Path = DEFAULT_CONFIG_PATH) -> None:
        ...
        if not path_obj.exists():
            ...
            try:
                path_obj.write_text(json.dumps(default_data, indent=4), encoding="utf-8")
                logger.info(f"MCPSR: Created default '{config_path}' configuration.")
            except Exception as e:
                logger.error(f"MCPSR: Failed to create default '{config_path}': {e}")
