...
try:
    from .live2d_model import Live2dModel  # type: ignore
except Exception:  # pragma: no cover - optional dependency removed
    Live2dModel = None  # type: ignore
...
class ServiceContext:
    ...
    def init_live2d(self, live2d_model_name: str) -> None:
        ...
        if Live2dModel is None:
            logger.warning("Live2D runtime unavailable; skipping Live2D initialization.")
            self.live2d_model = None
            return
    ...
    async def construct_system_prompt(self, persona_prompt: str) -> str:
        ...
        tool_prompts = (
            self.system_config.tool_prompts if self.system_config is not None else {}
        )
        for prompt_name, prompt_file in tool_prompts.items():
            ...
            if prompt_name == "live2d_expression_prompt":
                expressions = (
                    ...
                    if self.live2d_model is not None and hasattr(self.live2d_model, "emo_str")
                    else "[neutral], [anger], [disgust], [fear], [joy], [smirk], [sadness], [surprise]"
                )
                prompt_content = prompt_content.replace(
                    "[<insert_emomap_keys>]", expressions
                )
